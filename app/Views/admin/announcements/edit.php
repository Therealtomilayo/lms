<?php
/**
 * Admin Edit Announcement View (ADMIN-22)
 *
 * @var string $title
...
$publishedAtVal = $announcement->publishedAt ? date('Y-m-d\TH:i', strtotime($announcement->publishedAt)) : '';
$expiresAtVal = $announcement->expiresAt ? date('Y-m-d\TH:i', strtotime($announcement->expiresAt)) : '';
...
<div class="max-w-5xl mx-auto space-y-6">
        ...
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            <div>
                ...
                <nav class="flex items-center gap-2 text-xs font-semibold text-slate-400 uppercase tracking-wider mb-1" aria-label="Breadcrumb">
                    ...
                    <span class="text-brand-600">Edit Notice #<?= (int)$announcement->id ?></span>
                </nav>
                <h1 class="text-2xl font-bold text-slate-900 tracking-tight">Edit Institutional Announcement</h1>
                <p class="text-sm text-slate-500 mt-1">
                    Update announcement headline, modify body text, adjust audience targeting, or alter publication schedules.
                </p>
            </div>
    ...
    <!-- Main Grid Layout: Form + Announcement Summary Sidebar -->
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div class="lg:col-span-2 bg-white rounded-xl border border-slate-200 shadow-xs p-6 md:p-8">
            <form method="POST" action="/admin/announcements/<?= (int)$announcement->id ?>/edit" id="editAnnouncementForm" class="space-y-6">
                ...
                <div class="space-y-4 pb-6 border-b border-slate-100">
                    <div class="flex items-center gap-2">
                        ...
                        <div>
                            ...
                            <p class="text-xs text-slate-500">Modify target audience and visibility</p>
                        </div>
                    </div>
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 pt-2">
                        ...
                        <div>
                            ...
                            >
                                <option value="school" <?= $announcement->scope === 'school' ? 'selected' : '' ?>>School-wide (All Portals)</option>
                                <option value="class" <?= $announcement->scope === 'class' ? 'selected' : '' ?>>Specific Class</option>
                                <option value="class_subject" <?= $announcement->scope === 'class_subject' ? 'selected' : '' ?>>Specific Subject Group</option>
                            </select>
                        </div>
                        ...
                        <div id="subjectSelectWrap" class="<?= $announcement->scope === 'class_subject' ? '' : 'hidden' ?>">
                            ...
                            >
                                ...
                                <?php foreach ($classSubjects as $cs): ?>
                                    <option value="<?= (int)$cs->id ?>" <?= ($announcement->scope === 'class_subject' && (int)$announcement->scopeId === (int)$cs->id) ? 'selected' : '' ?>>
                                        <?= htmlspecialchars($cs->className ?? 'Class') ?> &mdash; <?= htmlspecialchars($cs->subjectName ?? 'Subject') ?>
                                    </option>
                                <?php endforeach; ?>
                            </select>
                        </div>
                ...
                <div class="space-y-4 pb-6 border-b border-slate-100">
                    <div class="flex items-center gap-2">
                        ...
                        <div>
                            ...
                            <p class="text-xs text-slate-500">Edit headline and body message</p>
                        </div>
                    </div>
                    ...
                    <div>
                        ...
                        <input
                            ...
                            placeholder="e.g., 2026/2027 First Term Examination Timetable"
                            class="w-full px-4 py-2.5 bg-slate-50/50 border border-slate-300 rounded-lg text-sm text-slate-800 placeholder-slate-400 focus:bg-white focus:outline-none focus:ring-2 focus:ring-brand-500 focus:border-brand-500 transition-colors font-medium"
                        >
                    </div>
                    ...
                    <div>
                        ...
                        <textarea
                            ...
                            class="w-full px-4 py-2.5 bg-slate-50/50 border border-slate-300 rounded-lg text-sm text-slate-800 placeholder-slate-400 focus:bg-white focus:outline-none focus:ring-2 focus:ring-brand-500 focus:border-brand-500 transition-colors leading-relaxed"
                ...
                <div class="space-y-4">
                    <div class="flex items-center gap-2">
                        ...
                        <div>
                            ...
                            <p class="text-xs text-slate-500">Configure publication timing and expiration</p>
                        </div>
                    </div>
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 pt-1">
                        ...
                        <div>
                            ...
                            <input
                                ...
                                name="published_at"
                                id="published_at"
                                value="<?= htmlspecialchars($publishedAtVal) ?>"
                                class="w-full px-3.5 py-2.5 bg-slate-50/50 border border-slate-300 rounded-lg text-sm text-slate-800 focus:bg-white focus:outline-none focus:ring-2 focus:ring-brand-500 focus:border-brand-500 transition-colors"
                            >
                            <p class="text-[11px] text-slate-400 mt-1">Controls when the notice becomes visible in portal feeds</p>
                        </div>
                        ...
                        <div>
                            ...
                            <input
                                ...
                                name="expires_at"
                                id="expires_at"
                                value="<?= htmlspecialchars($expiresAtVal) ?>"
                                class="w-full px-3.5 py-2.5 bg-slate-50/50 border border-slate-300 rounded-lg text-sm text-slate-800 focus:bg-white focus:outline-none focus:ring-2 focus:ring-brand-500 focus:border-brand-500 transition-colors"
                            >
        ...
        <div class="space-y-6">
            ...
            <div class="bg-white rounded-xl border border-slate-200 shadow-xs p-6 space-y-4">
                ...
                <div class="space-y-2.5 text-xs text-slate-600 divide-y divide-slate-100">
                    <div class="flex items-center justify-between pt-1">
                        <span class="text-slate-400">Notice ID:</span>
                        <strong class="font-mono text-slate-800">#<?= (int)$announcement->id ?></strong>
                    </div>
                    <div class="flex items-center justify-between pt-2">
                        <span class="text-slate-400">Current Audience:</span>
                        <strong class="text-slate-800"><?= htmlspecialchars($announcement->targetName ?? 'School-wide') ?></strong>
                    </div>
                    <div class="flex items-center justify-between pt-2">
                        <span class="text-slate-400">Original Author:</span>
                        <strong class="text-slate-800"><?= htmlspecialchars($announcement->authorName ?? 'Administrator') ?></strong>
                    </div>
                    <?php if ($announcement->createdAt): ?>
                        <div class="flex items-center justify-between pt-2">
                            <span class="text-slate-400">Created:</span>
                            <span class="text-slate-700"><?= htmlspecialchars(date('M d, Y', strtotime($announcement->createdAt))) ?></span>
                        </div>
                    <?php endif; ?>
                </div>
            </div>

            <!-- Audience Scope Guidelines Box -->
            <div class="bg-white rounded-xl border border-slate-200 shadow-xs p-6 space-y-4">
                <div class="flex items-center gap-2">
                    ...
                    <h3 class="text-sm font-bold text-slate-900">Audience Scope Guide</h3>
                </div>
                ...
                <div class="space-y-3 text-xs text-slate-600 leading-relaxed">
                    ...
                    <div class="p-3 bg-slate-50 rounded-lg border border-slate-200 space-y-1">
                        ...
                        <p class="text-slate-500">Visible only to students enrolled in that specific class, their teachers, and linked parents.</p>
                    </div>
