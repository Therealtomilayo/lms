...
<head>
    ...
    <title><?= e($title ?? 'Teacher Portal — Claret LMS') ?></title>
...
<body class="h-full bg-slate-50 text-slate-800 flex">
    ...
    <aside class="w-64 bg-slate-900 text-slate-200 flex flex-col flex-shrink-0 min-h-screen">
        <div class="p-5 border-b border-slate-800 flex items-center gap-3">
            ...
            <div>
                ...
                <span class="text-xs text-emerald-400 font-medium tracking-wide uppercase">Teacher Portal</span>
            </div>
        </div>

        <nav class="flex-1 p-4 space-y-1.5 overflow-y-auto" aria-label="Main Teacher Navigation">
            <a href="/teacher/dashboard" class="flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm font-medium text-slate-300 hover:bg-slate-800 hover:text-white transition">
            ...
            <a href="/teacher/content" class="flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm font-medium text-slate-300 hover:bg-slate-800 hover:text-white transition">
                <svg class="w-5 h-5 text-slate-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 6.253v13m0-13C10.832 5.477 9.246 5 7.5 5S4.168 5.477 3 6.253v13C4.168 18.477 5.754 18 7.5 18s3.332.477 4.5 1.253m0-13C13.168 5.477 14.754 5 16.5 5c1.747 0 3.332.477 4.5 1.253v13C19.832 18.477 18.247 18 16.5 18c-1.746 0-3.332.477-4.5 1.253"/></svg>
                Learning Materials
            </a>

            <a href="/teacher/timetable" class="flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm font-medium text-slate-300 hover:bg-slate-800 hover:text-white transition">
        ...
        <div class="p-4 border-t border-slate-800 bg-slate-950 flex items-center justify-between">
            <div class="flex items-center gap-3 min-w-0">
                <div class="w-8 h-8 rounded-full bg-emerald-700 flex items-center justify-center text-white font-bold text-xs flex-shrink-0">
                    <?= e(substr($_SESSION['user_name'] ?? 'T', 0, 1)) ?>
                </div>
                <div class="truncate">
                    <p class="text-xs font-medium text-white truncate"><?= e($_SESSION['user_name'] ?? 'Teacher') ?></p>
    ...
    <div class="flex-1 flex flex-col min-h-screen overflow-hidden">
        ...
        <header class="bg-white border-b border-slate-200 px-8 py-4 flex items-center justify-between flex-shrink-0">
            <div>
                <h1 class="text-xl font-bold text-slate-900 leading-tight"><?= e($headerTitle ?? 'Teacher Portal') ?></h1>
                <p class="text-xs text-slate-500"><?= e($headerSubtitle ?? 'Manage classroom materials and learning resources') ?></p>
            </div>
        ...
        <main id="main-content" class="flex-1 overflow-y-auto p-8 bg-slate-50">
            ...
            <?php if (!empty($_SESSION['flash_errors']) && is_array($_SESSION['flash_errors'])): ?>
                <div class="mb-6 p-4 rounded-xl bg-rose-50 border border-rose-200 text-rose-800 shadow-sm">
                    <p class="font-bold text-sm mb-2">Please correct the following errors:</p>
                    <ul class="list-disc list-inside text-xs space-y-1">
                        <?php foreach ($_SESSION['flash_errors'] as $field => $errs): ?>
