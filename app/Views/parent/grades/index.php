<div class="space-y-6">
    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
            <div class="flex items-center gap-2">
                <a href="/parent/dashboard" class="text-sm font-semibold text-slate-500 hover:text-slate-800 transition">&larr; Back to Guardian Portal</a>
            </div>
            <h2 class="text-2xl font-bold text-slate-900 tracking-tight mt-1">
                Academic Performance &mdash; <?= e($student->user?->name ?? 'Child') ?>
            </h2>
            <p class="text-sm text-slate-500 mt-0.5">
                Admission No: <?= e($student->admissionNumber) ?>
            </p>
        </div>
        <?php if ($isPublished && !empty($subjectResults)): ?>
            <div>
                <a href="/parent/children/<?= $student->id ?>/grades/report-card?term_id=<?= $selectedTermId ?>" target="_blank" class="inline-flex items-center gap-2 px-4 py-2.5 bg-brand-600 hover:bg-brand-700 text-white text-sm font-semibold rounded-xl shadow-sm transition">
    ...
    <div class="bg-white rounded-2xl border border-slate-200 shadow-sm p-4">
        <form method="GET" action="/parent/children/<?= $student->id ?>/grades" class="flex items-center gap-4">
            <div>
    ...
    <?php if (!$isPublished): ?>
        <div class="bg-amber-50 border border-amber-200 rounded-2xl p-8 text-center text-amber-800">
            ...
            <h3 class="text-base font-bold">Results Not Yet Published</h3>
            <p class="text-xs mt-1 text-amber-700 max-w-md mx-auto">Official grades for this academic term have not yet been approved and published by school administration.</p>
        </div>
    <?php else: ?>
        ...
        <div class="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
            <div class="p-5 border-b border-slate-100">
                <h3 class="text-base font-bold text-slate-900">Academic Subjects</h3>
            </div>
