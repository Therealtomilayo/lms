...
<div class="space-y-6">
    ...
    <div class="bg-white rounded-2xl border border-slate-200 shadow-xs p-6">
        <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
            <div class="flex items-center gap-4">
                ...
                <div>
                    <h1 class="text-2xl font-extrabold text-slate-900 tracking-tight">Class Discussions Hub</h1>
                    <p class="text-xs text-slate-500 mt-1">Select a class subject group to manage threaded academic discussions, review questions, and moderate peer dialogues.</p>
                </div>
            </div>
    ...
    <?php if (empty($classSubjects)): ?>
        <div class="bg-white rounded-2xl border border-slate-200 p-12 text-center shadow-xs">
            ...
            <h3 class="text-base font-extrabold text-slate-800">No Teaching Allocations Found</h3>
            <p class="text-xs text-slate-500 mt-1">You are not currently assigned as the subject instructor for any active class subjects.</p>
        </div>
    <?php else: ?>
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
            <?php foreach ($classSubjects as $cs): 
                $subName = $cs->subject?->name ?? 'Subject';
                $cName = $cs->schoolClass?->getFullName() ?? ($cs->schoolClass?->name ?? 'Class');
                ...
                <div class="bg-white rounded-2xl border border-slate-200 p-5 shadow-xs flex flex-col justify-between hover:shadow-md hover:border-brand-300 transition duration-200 group">
                    <div>
                        <div class="flex items-center justify-between gap-2 mb-3">
                            <span class="inline-flex items-center px-2.5 py-0.5 rounded-lg text-xs font-extrabold bg-brand-50 text-brand-700 border border-brand-200">
                        ...
                        <p class="text-xs text-slate-500 mt-1 leading-relaxed">
                            Engage with pupils, post academic inquiries, and moderate group discussions for this course.
                        </p>
                    </div>
                    ...
                    <div class="mt-5 pt-4 border-t border-slate-100 flex items-center justify-between">
                        ...
                        <a href="/teacher/subjects/<?= (int)$cs->id ?>/discussions" 
