...
<div class="space-y-6">
    ...
    <div class="bg-white rounded-2xl border border-slate-200 shadow-xs p-6">
        <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
            <div>
                <nav class="flex items-center gap-2 text-xs font-semibold text-slate-500 uppercase tracking-wider mb-2">
                    ...
                    <span class="text-slate-700">Class Behavioral Evaluation</span>
                </nav>
                <div class="flex flex-wrap items-center gap-3">
                    <h1 class="text-2xl font-extrabold text-slate-900 tracking-tight leading-tight">
                        Batch Remarks & Behavioral Ratings
                    </h1>
                </div>
                <p class="text-xs text-slate-500 mt-1">
                    Author termly teacher remarks with quick-suggestion templates and evaluate students on psychomotor & affective traits.
                </p>
            </div>
            <div>
                <a href="/teacher/gradebook" class="inline-flex items-center gap-2 px-4 py-2 text-xs font-bold text-slate-700 bg-slate-100 hover:bg-slate-200 rounded-xl transition border border-slate-200">
        ...
        <div class="mt-6 pt-5 border-t border-slate-100">
            <form method="GET" action="/teacher/results/comments" id="teacher-cohort-form" class="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div>
                    <label class="block text-xs font-bold text-slate-700 uppercase tracking-wider mb-1.5">Academic Session</label>
                    <select name="session_id" onchange="document.getElementById('teacher-cohort-form').submit()" 
                            class="w-full px-3.5 py-2 text-sm rounded-xl border border-slate-300 focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500 outline-none transition bg-white">
                ...
                <div>
                    ...
                    <select name="term_id" onchange="document.getElementById('teacher-cohort-form').submit()" 
                            class="w-full px-3.5 py-2 text-sm rounded-xl border border-slate-300 focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500 outline-none transition bg-white">
            ...
            <div class="flex items-center justify-between flex-wrap gap-4 bg-white p-4 rounded-xl border border-slate-200 shadow-sm">
                <div class="flex items-center gap-3">
                    <span class="px-3 py-1 bg-emerald-100 text-emerald-800 font-bold text-xs rounded-full">
                        <?= count($students) ?> Students in <?= htmlspecialchars($selectedClass?->name ?? 'Class') ?>
                    </span>
                ...
                <div class="flex items-center gap-3">
                    ...
                    <button type="submit" class="inline-flex items-center gap-2 px-5 py-2 bg-emerald-600 hover:bg-emerald-700 text-white font-bold text-xs rounded-xl shadow-sm transition">
                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 7H5a2 2 0 00-2 2v9a2 2 0 002 2h14a2 2 0 002-2V9a2 2 0 00-2-2h-3m-1 4l-3 3m0 0l-3-3m3 3V4"/></svg>
                        <span>Save Teacher Remarks & Ratings</span>
                    </button>
                </div>
            </div>
                        ...
                        <div class="p-4 grid grid-cols-1 lg:grid-cols-2 gap-4">
                            ...
                            <div class="space-y-2">
                                <div class="flex items-center justify-between flex-wrap gap-2">
                                    ...
                                    <select onchange="applyPreset(this, 'tc-<?= $studentId ?>', '<?= addslashes(htmlspecialchars($studentName)) ?>')" 
                                            class="text-xs px-2.5 py-1 rounded-lg border border-slate-200 bg-slate-50 hover:bg-white text-slate-600 focus:outline-none focus:ring-1 focus:ring-emerald-500">
                                        <option value="">⚡ Quick Suggestion...</option>
                                        <?php foreach ($teacherPresets as $pr): ?>
                                            <option value="<?= htmlspecialchars($pr->text, ENT_QUOTES) ?>">
                                                [<?= ucfirst(htmlspecialchars($pr->category)) ?>] <?= substr(htmlspecialchars($pr->text), 0, 45) ?>...
                                            </option>
                                        <?php endforeach; ?>
                                    </select>
                                </div>
                                ...
                                <textarea name="comments[<?= $studentId ?>][teacher_remark]" 
                                          id="tc-<?= $studentId ?>" 
                                          rows="3" 
                                          placeholder="Type form teacher remarks or select from quick suggestions..."
                                          class="w-full px-3.5 py-2 text-sm rounded-xl border border-slate-200 focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500 outline-none transition leading-relaxed"><?= htmlspecialchars($teacherComment) ?></textarea>
                            </div>

                            <!-- Principal Comment Status (Read-only for faculty) -->
                            <div class="space-y-2">
                                <label class="text-xs font-bold text-slate-700 uppercase tracking-wider flex items-center gap-1.5">
                                    <span class="w-2 h-2 rounded-full bg-purple-500"></span>
                                    Principal / Head of School Endorsement
                                </label>
                                <div class="p-3 bg-slate-50 rounded-xl border border-slate-200 text-xs text-slate-700 min-h-[76px] leading-relaxed italic">
                                    <?php if (!empty($principalComment)): ?>
                                        &ldquo;<?= htmlspecialchars($principalComment) ?>&rdquo;
                                    <?php else: ?>
                                        <span class="text-slate-400 font-normal">Pending principal terminal endorsement review.</span>
                                    <?php endif; ?>
                                </div>
                            </div>
                        </div>

                        <!-- Collapsible Ratings Drawer -->
                        <div id="ratings-drawer-<?= $studentId ?>" class="hidden border-t border-slate-200 bg-gradient-to-b from-slate-50 to-white p-5 space-y-4">
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
                                <?php foreach ($skills as $sk): 
                                    $skillId = (int)$sk->id;
                                    $currentScore = $ratings[$skillId] ?? 4;
                                ?>
                                    <div class="bg-white p-2.5 rounded-xl border border-slate-200 flex items-center justify-between gap-2 shadow-2xs">
                                        <div class="text-xs font-medium text-slate-800 truncate" title="<?= htmlspecialchars($sk->name) ?>">
                                            ...
                                            <?php for ($val = 1; $val <= 5; $val++): ?>
                                                <label class="cursor-pointer">
                                                    <input type="radio" 
                                                           ...
                                                           class="sr-only peer">
                                                    <span class="w-6 h-6 flex items-center justify-center rounded-lg text-xs font-bold transition peer-checked:bg-emerald-600 peer-checked:text-white bg-slate-100 text-slate-600 hover:bg-slate-200">
                                                        <?= $val ?>
                                                    </span>
                                                </label>
                                            <?php endfor; ?>
            ...
            <div class="sticky bottom-4 bg-slate-900/95 backdrop-blur text-white p-4 rounded-2xl shadow-xl flex items-center justify-between flex-wrap gap-4 border border-slate-800">
                <div class="flex items-center gap-3">
                    <span class="text-xs text-slate-300">
                        Ready to save remarks for <strong><?= count($students) ?> students</strong> in <?= htmlspecialchars($selectedClass?->name ?? 'Class') ?>.
                    </span>
                </div>

                <div class="flex items-center gap-3">
                    <button type="submit" class="inline-flex items-center gap-2 px-6 py-2.5 bg-emerald-600 hover:bg-emerald-500 text-white font-bold text-sm rounded-xl shadow-md transition">
